...
class newDisplay {
    ...
    public function get_search_html() {
        ...
        $html .= ' <div class="faqwd_search">
                    <input type="text" class="faqwd_search_input" placeholder="Search for answers..."/>
    ...
    public function get_cats_html() {
        ...
        if (count($this->cats) > 0) {
            ...
            foreach ($this->cats as $cat_id => $cat) {
                ...
                $image_id = get_term_meta ( $cat_id, 'faq-category-image-id', true );
                $image = '';
                if($image_id) {
                ...
                if($image != '') {
                    $html .= '<span class="faqwd_category_image">'.$image.'</span>';
                }
                $html .= $cat->name;
            }
            $html .= '</li>';
    ...
    public function get_questions() {
        ...
        $html .= '<h1>Topics and questions</h1>';
        ...
        foreach ($this->cats as $cat_id => $cat) {
            if (isset($posts[$cat_id]) && count($posts[$cat_id]) > 0) {
                $fa_cat_id = get_option('faq_category_'.$cat_id.'_category_id');
                $html .= '<div class="faqwd_cat_' . $cat_id . ' faqwd_cat"  id="'.$fa_cat_id.'">';
                $html .= '<ul class="faqwd_questions_ul">';
                $html .= '<li class="faqwd_category_head_li" id="faqwd_category_head_li_' . $cat_id .'">';
                $cat_heading = $cat->name;
                if (isset($cat->description) && $cat->description !== "") {
                    $cat_heading = $cat->description;
                }
                 
                $html .= '<div class="faqwd_category_head">'.$cat_heading.'</div>';
                $html .= '</li >';
                ...
                foreach ($posts[$cat_id] as $number => $post) {
                    ...
                    $html .= '</div>';
                    $html .= '<div class="faqwd_question_content faqwd_hidden faqwd_question_' . $cat_id . '_' . $post->ID . '">';
                ...
                $html .= '<li class="faqwd_scroll_top">';
                $html .= '<a class="faqwd_category_scroll" href="#">Go to the top <i class="fa fa-chevron-up" aria-hidden="true"></i></a>';
                $html .= '</li >';
                $html .= '</ul > ';
                $html .= '</div > ';
            }
